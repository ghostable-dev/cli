...
import type { EnvironmentSecretBundle } from '@/domain'; // domain result (optional type)
...
export function registerDeployCloudCommand(program: Command) {
	program
		...
		.action(async (opts: { token?: string; out?: string; only?: string[] }) => {
			...
			// 2) Fetch bundle for the current environment context (derived from token)
